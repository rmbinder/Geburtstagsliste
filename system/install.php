...
try {
    ...
    else
    {
        ...
        $role->readDataByColumns(array('rol_cat_id' => $categoryCommonId, 'rol_name' => $gL10n->get('PLG_BIRTHDAYLIST_ACCESS_TO_PLUGIN'), 'rol_description' => $gL10n->get('PLG_BIRTHDAYLIST_ACCESS_TO_PLUGIN_DESC')));
        ...
        if ($role->getValue('rol_id') === 0)                 // nur wenn es keine Rolle gibt, neue Daten eingeben (mehr als eine Rolle wird nicht betrachtet)
        {
            ...
            $role->setValue('rol_name', $gL10n->get('PLG_BIRTHDAYLIST_ACCESS_TO_PLUGIN'));
            $role->setValue('rol_description', $gL10n->get('PLG_BIRTHDAYLIST_ACCESS_TO_PLUGIN_DESC'));
            $role-> save();
        }
        ...
        $menu->setValue('men_icon', 'cake2');
        $menu->setValue('men_name', 'PLG_BIRTHDAYLIST_NAME');
        $menu->setValue('men_description', 'PLG_BIRTHDAYLIST_NAME_DESC');
        $menu->save();
