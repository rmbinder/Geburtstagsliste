...
try {
    ...
    $monate = array(
        '00' => $gL10n->get('PLG_BIRTHDAYLIST_ALL_MONTHS'),
    ...
    $getConfig = admFuncVariableIsValid($_GET, 'config', 'string', array(
        'defaultValue' => 'X' . $pPreferences->config['Optionen']['config_default'] . 'X',
        'validValues' => $validValues
    ));

    $validValues = array(
        0 => 'X' . $pPreferences->config['Optionen']['vorschau_tage_default'] . 'X'
    );
    ...
    $getMonth = admFuncVariableIsValid($_GET, 'month', 'string', array(
        'defaultValue' => '00',
        'validValues' => array(
            '00',
            '01',
    ...
    $getExportAndFilter = admFuncVariableIsValid($_GET, 'export_and_filter', 'bool', array(
        'defaultValue' => false
    ));
    ...
    if ($getMode != 'csv' && $getMode != 'xlsx') {
        ...
        if ($getMode == 'print') {
            ...
            $table->addColumn($subheadline, array(
            ...
            $page->addJavascript('
                $("#previewList").change(function () {
                    if($(this).val().length > 1) {
                        self.location.href = "' . SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . PLUGIN_FOLDER . '/system/birthday_list.php', array(
                'mode' => 'html',
                'month' => $getMonth,
                'filter' => $getFilter,
                ...
                $("#menu_item_lists_print_view").click(function() {
                    window.open("' . SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . PLUGIN_FOLDER . '/system/birthday_list.php', array(
                'previewdays' => $getPreviewDays,
                'filter' => $getFilter,
                'export_and_filter' => $getExportAndFilter,
                'month' => $getMonth,
            ...
            if ($getExportAndFilter) {
                ...
                $page->addPageFunctionsMenuItem('menu_item_lists_xlsx', $gL10n->get('SYS_MICROSOFT_EXCEL') . ' (XLSX)', SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . PLUGIN_FOLDER . '/system/birthday_list.php', array(
                    'previewdays' => $getPreviewDays,
                    'filter' => $getFilter,
                    'export_and_filter' => $getExportAndFilter,
                    'month' => $getMonth,
                ...
                $page->addPageFunctionsMenuItem('menu_item_lists_csv_ms', $gL10n->get('SYS_MICROSOFT_EXCEL') . ' (CSV)', SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . PLUGIN_FOLDER . '/system/birthday_list.php', array(
                    'previewdays' => $getPreviewDays,
                    'filter' => $getFilter,
                    'export_and_filter' => $getExportAndFilter,
                    'month' => $getMonth,
                ...
                $page->addPageFunctionsMenuItem('menu_item_lists_pdf', $gL10n->get('SYS_PDF') . ' (' . $gL10n->get('SYS_PORTRAIT') . ')', SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . PLUGIN_FOLDER . '/system/birthday_list.php', array(
                    'previewdays' => $getPreviewDays,
                    'filter' => $getFilter,
                    'export_and_filter' => $getExportAndFilter,
                    'month' => $getMonth,
                ...
                $page->addPageFunctionsMenuItem('menu_item_lists_pdfl', $gL10n->get('SYS_PDF') . ' (' . $gL10n->get('SYS_LANDSCAPE') . ')', SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . PLUGIN_FOLDER . '/system/birthday_list.php', array(
                    'previewdays' => $getPreviewDays,
                    'filter' => $getFilter,
                    'export_and_filter' => $getExportAndFilter,
                    'month' => $getMonth,
                ...
                $page->addPageFunctionsMenuItem('menu_item_lists_csv', $gL10n->get('SYS_CSV') . ' (' . $gL10n->get('SYS_UTF8') . ')', SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . PLUGIN_FOLDER . '/system/birthday_list.php', array(
                    'previewdays' => $getPreviewDays,
                    'filter' => $getFilter,
                    'export_and_filter' => $getExportAndFilter,
                    'month' => $getMonth,
                ...
                $page->addPageFunctionsMenuItem('menu_item_ical', $gL10n->get('PLG_BIRTHDAYLIST_ICAL'), SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . PLUGIN_FOLDER . '/system/ical_export.php', array(
                    'previewdays' => $getPreviewDays,
                    'config' => $getConfig,
                    'month' => $getMonth,
                    'filter' => $getFilter
            ...
            $form = new FormPresenter('navbar_birthdaylist_form', '../templates/form.filter.without.text.filter.plugin.birthdaylist.tpl', SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . PLUGIN_FOLDER . '/system/birthday_list.php', array(
                'headline' => $headline
            )), $page, array(
                'type' => 'navbar',
                'setFocus' => false
            ...
            $selectBoxEntries = array(
                ' ' => $gL10n->get('PLG_BIRTHDAYLIST_SELECT_CONFIGURATION') . ' ...'
            );
            ...
            $selectBoxEntries = array(
                '' => $gL10n->get('PLG_BIRTHDAYLIST_SELECT_NUMBER_OF_DAYS') . ' ...'
            );
            foreach ($pPreferences->config['Optionen']['vorschau_liste'] as $item) {
            ...
            $selectBoxEntries = array(
                '' => $gL10n->get('PLG_BIRTHDAYLIST_SELECT_MONTH') . ' ...'
            );
    ...
    foreach ($liste->headerData as $columnHeader) {
        ...
        if ($gProfileFields->getPropertyById($usf_id, 'usf_type') == 'CHECKBOX' || $gProfileFields->getPropertyById($usf_id, 'usf_name_intern') == 'GENDER') {
            $columnAlign[] = 'center';
        } elseif ($gProfileFields->getPropertyById($usf_id, 'usf_type') == 'NUMBER' || $gProfileFields->getPropertyById($usf_id, 'usf_type') == 'DECIMAL_NUMBER') {
            $columnAlign[] = 'right';
        } else {
            $columnAlign[] = 'left';
        }
        ...
        if ($getMode == 'csv') {
            ...
            if ($columnNumber == 1) {
                $table->addColumn($gL10n->get('SYS_ABR_NO'), array(
                    'style' => 'text-align: left;font-size:14;background-color:#C7C7C7;'
                ), 'th');
            }
            $table->addColumn($columnHeader['data'], array(
                'style' => 'text-align: left;font-size:14;background-color:#C7C7C7;'
            ), 'th');
        } elseif ($getMode == 'xlsx') {
            if ($columnNumber == 1) {
    ...
    foreach ($liste->listData as $memberdata) {
        ...
        for ($i = 1; $i < count($memberdata); $i ++) {
            ...
            if ($usf_id != 0 && (in_array($getMode, array(
                'xlsx',
                'pdf',
                'csv'
                    ...
                    if ($gProfileFields->getPropertyById($usf_id, 'usf_type') == 'EMAIL') {
                        if ($gSettingsManager->getInt('mail_module_enabled') !== 1) {
                            ...
                            $mail_link = SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . PLUGIN_FOLDER . '/system/message_write.php', array(
                                ...
                                'config' => trim($getConfig, 'X'),
                                'configtext' => end($memberdata)
                            ));
                        }
                        $columnValues[] = '<a href="' . $mail_link . '">' . $content . '</a><br />';
                    } elseif ($getMode === 'html' && ($gProfileFields->getPropertyById($usf_id, 'usf_name_intern') == 'LAST_NAME' || $gProfileFields->getPropertyById($usf_id, 'usf_name_intern') == 'FIRST_NAME') && substr($liste->headerData[$i]['id'], 0, 1) != 'r') {
        ...
        if ($getFilter == '' || ($getFilter != '' && (stristr(implode('', $columnValues), $getFilter) || stristr($tmp_csv, $getFilter)))) {
            if ($getMode == 'csv') {
                ...
                $table->addRowByArray($columnValues, '', array(
                    'nobr' => 'true'
                ));
            }
            $listRowNumber ++;
    ...
    elseif ($getMode == 'pdf') {
        ...
        try {
            ...
            $gLogger->error('Could not delete file!', array(
                'filePath' => $file
            ));
            // TODO
        }
        ...
        $writer->setKeywords(array(
            $gL10n->get('PLG_BIRTHDAYLIST_NAME'),
            $gL10n->get('PLG_BIRTHDAYLIST_PATTERN')
        ));
