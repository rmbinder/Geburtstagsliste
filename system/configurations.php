...
try {
    ...
    if ($getDelete > 0) {
        ...
        $gDb->queryPrepared($sql, array(
            'PGLMAIL_NOTIFICATION' . ($getDelete - 1),
            $gCurrentOrgId
        ));
        ...
        for ($i = $getDelete; $i < $num_configs; $i ++) {
            $sql = 'UPDATE ' . TBL_TEXTS . '
                 ...
                 WHERE txt_name = ?
            	   AND txt_org_id = ? ';
            $gDb->queryPrepared($sql, array(
                'PGLMAIL_NOTIFICATION' . ($i - 1),
                'PGLMAIL_NOTIFICATION' . $i,
                $gCurrentOrgId
            ));
        }
    ...
    if ($getCopy > 0) {
        ...
        $textCopy->readDataByColumns(array(
            'txt_name' => 'PGLMAIL_NOTIFICATION' . $getCopy - 1,
            'txt_org_id' => $gCurrentOrgId
        ));
        $value = $textCopy->getValue('txt_text');
        $textCopy->readDataByColumns(array(
            'txt_name' => 'PGLMAIL_NOTIFICATION' . count($pPreferences->config['Konfigurationen']['col_desc']) - 1,
            'txt_org_id' => $gCurrentOrgId
        ));
    ...
    $formConfigurations = new FormPresenter('adm_configurations_preferences_form', '../templates/configurations.plugin.birthdaylist.tpl', SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . '/BirthdayList/system/configurations_function.php', array(
        'form' => 'configurations'
    )), $page, array(
    ...
    for ($conf = 0; $conf < $num_configs; $conf ++) {
        ...
        $formConfigurations->addSelectBoxFromSql('selection_role' . $conf, $gL10n->get('SYS_ROLE_SELECTION'), $gDb, $sql, array(
            'defaultValue' => explode(',', $pPreferences->config['Konfigurationen']['selection_role'][$conf]),
            'multiselect' => true
        ));
        ...
        $formConfigurations->addSelectBoxFromSql('selection_cat' . $conf, $gL10n->get('SYS_CAT_SELECTION'), $gDb, $sql, array(
            'defaultValue' => explode(',', $pPreferences->config['Konfigurationen']['selection_cat'][$conf]),
            'multiselect' => true
        ));

        $text[$conf] = new Text($gDb);
        $text[$conf]->readDataByColumns(array(
            'txt_name' => 'PGLMAIL_NOTIFICATION' . $conf,
            'txt_org_id' => $gCurrentOrgId
        ));
        ...
        $formConfigurations->addInput('years_offset' . $conf, $gL10n->get('PLG_BIRTHDAYLIST_YEARS_OFFSET'), $pPreferences->config['Konfigurationen']['years_offset'][$conf], array(
            'type' => 'number',
            'step' => 1,
            'minNumber' => - 99,
            'maxNumber' => 99
        ...
        $sql = 'SELECT urt_id, urt_name
                  FROM ' . TBL_USER_RELATION_TYPES . '
          		 ORDER BY urt_name';
        $formConfigurations->addSelectBoxFromSql('relationtype_id' . $conf, $gL10n->get('PLG_BIRTHDAYLIST_RELATION'), $gDb, $sql, array(
            'defaultValue' => $pPreferences->config['Konfigurationen']['relation'][$conf],
            'showContextDependentFirstEntry' => true,
            'multiselect' => false
        ));

        if ($num_configs > 1) {
            $configuration['urlConfigDelete'] = SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . '/BirthdayList/system/configurations.php', array(
                'delete' => $conf + 1
            ));
        }
        if (! empty('desc' . $conf)) {
            $configuration['urlConfigCopy'] = SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . '/BirthdayList/system/configurations.php', array(
                'copy' => $conf + 1
            ));
        }
    ...
    $page->assignSmartyVariable('urlConfigNew', SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . '/BirthdayList/system/configurations.php', array(
        'add' => 1
    )));
    $page->assignSmartyVariable('urlPopupText', SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . '/BirthdayList/system/configurations_popup.php', array(
