...
try {
    ...
    switch ($getMode) {
        case 'security_check':
            ...
            $page = PagePresenter::withHtmlIDAndHeadline('plg-birthdaylist-uninstall-html');
            $page->setTitle($title);
            $page->setHeadline($headline);

            $form = new FormPresenter('birthdaylist_uninstall_form', '../templates/uninstall.plugin.birthdaylist.tpl', '', $page, array(
                'type' => 'default',
                'method' => 'post',
                'setFocus' => false
            ));
        ...
        case 'uninst':
            ...
            if ($pPreferences->config['install']['access_role_id'] == 0 || $pPreferences->config['install']['menu_item_id'] == 0) {
                ...
                $result .= $gL10n->get('PLG_BIRTHDAYLIST_UNINST_NO_INST_IDS_FOUND');
            } else {
                $result_role = true;
                ...
                if (count($access_roles_prefs) === 1 && $access_roles_prefs[0] == $pPreferences->config['install']['access_role_id']) {
                    ...
                    $result .= $gL10n->get('PLG_BIRTHDAYLIST_UNINST_MENU_ITEM_NOT_DELETED');
                }
            }
